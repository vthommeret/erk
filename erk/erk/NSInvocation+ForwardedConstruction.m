          ...
          invokeOnMainThread:(BOOL)mainThread
{
    ...
    instance->invokeOnMainThread = mainThread;
}
...
+ (id)invokeOnMainThreadWithTarget:(id)target {
    NSInvocation *invocation;
    return [self invocationWithTarget:target invocationOut:&invocation retainArguments:NO invokeOnMainThread:YES];
}
